...
module.exports.request = function(requestOptions, options, body) {
  ...
  var req = handler.request(requestOptions, function(response) {
    // If the response code is greater or equal to 400, and is not
    // in the ignore list, reject the promise with the response as
    // the parameter.
    if(response.statusCode >= 400 && options.ignoredErrorCodes.indexOf(response.statusCode) < 0) {
      deferred.reject(new Error('Response code ' + response.statusCode + ' was received but it was not in the ignore list.'));
    }
    ...
    response.on('end', function() {
          ...
          || options.ignoredErrorCodes.indexOf(response.statusCode) > -1)
      {
        ...
        deferred.reject(new Error('Request returned status code ' + response.statusCode + ' but it was not in the accepted list.'));
      }
    });
  });
